...
- One major design flaw in the MCP spec is that it only supports two communication channels: STDIO and SSE (server-side events.)  Server Side Events function like unidirection WebSockets (they stream from the server to the client.) In actual practice, tool usage tends to take the form of atomic bits of data (messages) as opposted to streaming a continuing conversation.
